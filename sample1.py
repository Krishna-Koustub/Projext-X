...
def msg():
    ...
    if(select == 1 or select == 2):
        ...
        if (e1.index("end") == 0):
            ...
            mb.showinfo('Success', 'Registration done successfully  for the vehicle : '+ course)
    else:
            mb.showinfo('Missing details', 'enter  age')
...
def save():
    ...
    s='\n'+now.strftime("%d-%m-%Y %H:%M")+'\t'+e1.get()+'\t'+e2.get()+'\t '+e3.get()+'\t'+ d+'\t'+age+' \t'+course
    ...
    with open('Regfile.csv', 'a') as fs:
        ...
        w.writerow([now.strftime("%d-%m-%Y %H:%M"), e1.get(),e2.get(),e3.get(),d,age,course])
        fs.close()
...
l1 = Label(root, text="Garage details ",width=25,font=("times",20,"bold"),bg='blue',fg='white')
l1.place(x=70,y=50)
...
l4 = Label(root, text="Date of purchase",width=20,font=("times",12,"bold"),anchor="w",bg='grey')
l4.place(x=70,y=230)
...
l5 = Label(root, text="age", width=20, font=("times",12,"bold"),anchor="w",bg='grey')
l5.place(x=70,y=280)

# radiobuttons
var = IntVar()
r1 = Radiobutton(root, text="new", variable=var, value=1, font=("times",12),bg='grey')
r1.place(x=235,y=280)
r2 = Radiobutton(root, text="old", variable=var, value=2, font=("times",12),bg='grey')
...
l7 = Label(root, text="Details",width=20,font=("times",12,"bold"),anchor="w",bg='grey')
l7.place(x=70,y=370)

# create a dropdown menu with the OptionMenu widget
cvar = StringVar()
cvar.set("Select Details")
option = ("X", "Y", "Z","A")
